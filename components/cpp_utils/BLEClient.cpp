 ...
 */
	...
	uint16_t count = 10;
	esp_gattc_service_elem_t *result = (esp_gattc_service_elem_t*)calloc(count, sizeof(esp_gattc_service_elem_t));
	esp_err_t errRc;
	...
	if(count < 2) {
		...
		for(int i=0; i<count;i++){
			...
			// while(m_servicesMapByInstID.find(pRemoteService) >= 0){
			// 	id.inst_id++;
			// }
			m_servicesMap.insert(std::pair<std::string, BLERemoteService *>(uuid.toString(), pRemoteService));
			// m_servicesMapByInstID.insert(std::pair<BLERemoteService *, uint16_t>(pRemoteService, id.inst_id));
			result++;
